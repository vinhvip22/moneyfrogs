...
image.addEventListener('click' , (e)=> {
    ...
    if(Number(power) > 0){
        localStorage.setItem('coins' , `${Number(coins) + 2}`);
        h1.textContent = `${(Number(coins) + 2).toLocaleString()}`;
    
        localStorage.setItem('power' , `${Number(power) - 5}`);
        body.querySelector('#power').textContent = `${Number(power) - 5}`;
    } 
